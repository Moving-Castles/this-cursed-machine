...
// Import user types
import { PORT_TYPE } from "./../Types.sol";

bytes32 constant _tableId = bytes32(abi.encodePacked(bytes16("mc"), bytes16("PortType")));
bytes32 constant PortTypeTableId = _tableId;
...
library PortType {
  ...
  function get(bytes32 key) internal view returns (PORT_TYPE value) {
    ...
    return PORT_TYPE(uint8(Bytes.slice1(_blob, 0)));
  }
  ...
  function encode(PORT_TYPE value) internal pure returns (bytes memory) {
