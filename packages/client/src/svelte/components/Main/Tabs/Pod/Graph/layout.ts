...
const FIXED_POSITIONS = {
    ...
    inletTwo: { x: 0, y: Math.floor((GRID.HEIGHT - (ORFICE.HEIGHT * 3)) / 2) + ORFICE.HEIGHT * 2 },
    outlet: { x: GRID.WIDTH - ORFICE.WIDTH, y: Math.floor((GRID.HEIGHT - ORFICE.HEIGHT) / 2) },
    player: { x: Math.floor((GRID.WIDTH - PLAYER.WIDTH) / 2), y: Math.floor((GRID.HEIGHT - PLAYER.HEIGHT) / 2) },
}
...
function getFreePosition(graphMachines: GraphMachines) {
    for (let i = 0; i < DYNAMIC_POSITIONS.length; i++) {
        ...
        if (positionIsFree(position, graphMachines)) {
            return {
                ...position, placementGroup: i < 5 ? PLACEMENT_GROUP.TOP : PLACEMENT_GROUP.BOTTOM
            }
        }
    }
...
function createGrid() {
    ...
    for (let x = 0; x < ORFICE.WIDTH; x++) {
        for (let y = 0; y < ORFICE.HEIGHT; y++) {
            grid = setWalkableAt(grid, FIXED_POSITIONS.inletOne.x + x, FIXED_POSITIONS.inletOne.y + y, false);
            grid = setWalkableAt(grid, FIXED_POSITIONS.inletTwo.x + x, FIXED_POSITIONS.inletTwo.y + y, false);
            grid = setWalkableAt(grid, FIXED_POSITIONS.outlet.x + x, FIXED_POSITIONS.outlet.y + y, false);
        }
    }
    ...
    for (let x = 0; x < PLAYER.WIDTH; x++) {
        for (let y = 0; y < PLAYER.HEIGHT; y++) {
            grid = setWalkableAt(grid, FIXED_POSITIONS.player.x + x, FIXED_POSITIONS.player.y + y, false);
        }
    }
...
function combineStates(simulatedMachines: SimulatedMachines, previousGraphMachines: GraphMachines): GraphMachines {
    ...
    for (const machineId in simulatedMachines) {
        ...
        if (previousGraphMachines.hasOwnProperty(machineId)) {
            ...
            graphMachines[machineId].placementGroup = previousGraphMachines[machineId].placementGroup
        } else {
...
export function createLayout(
    ...
    const graphMachines: GraphMachines = combineStates(simulatedMachines, previousGraphMachines)
