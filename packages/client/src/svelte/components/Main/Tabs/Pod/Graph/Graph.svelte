<script lang="ts">
  ...
  import { ENTITY_TYPE, MATERIAL_TYPE } from "@modules/state/base/enums"
  ...
  import { connectionState, machineState } from "@modules/state/simulated"
  ...
  $: {
    ...
    graph.nodes.forEach(newNode => {
      if (
        ...
        console.log("newNode.entry", newNode.entry)

        newNode.fy = newNode.entry.buildIndex === 1 ? -40 : 40
      }
