...
export function flicker(node: Element, { delay = 0, duration = 1000, easing = sineInOut } = {}) {
    ...
    return {
        ...
        css: (t: number) => {
            ...
            let flickerEffect = Math.sin(t * Math.PI * (Math.random() * frequencyVariance + baseFrequency)) *
                Math.sin(t * Math.PI * secondaryFrequency) * flickerAmplitude;
            // Opacity calculation with variability for more pronounced flickers
            let opacity = Math.min(Math.max(t + flickerEffect * (Math.random() + flickerVariability), 0), 1) * originalOpacity;
