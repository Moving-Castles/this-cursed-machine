...
export function createSyncProgressSystem() {
  let subscription = get(network).components.SyncProgress.update$.subscribe(update => {
    console.log("==> SyncProgress system: ", update);
    loadingMessage.set(update.value[0]?.message + ": " + update.value[0]?.percentage.toFixed(2));
