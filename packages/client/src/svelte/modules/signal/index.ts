...
export function initSignalNetwork() {
    ...
    //             if (msgObj.data.world !== get(publicNetwork).worldContract.address) return messages
...
async function sendVerification() {
    const signature = await get(publicNetwork).walletClient.signMessage({ message: MESSAGE });
